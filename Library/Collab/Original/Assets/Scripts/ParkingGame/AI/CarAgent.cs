...
namespace ParkingGame.AI
{
    public class CarAgent : Agent, IInput
    {
        ...
        public override void Initialize()
        {
            ...
            _last_distance = Vector3.Distance(_target.transform.position, gameObject.transform.position);
        }
    
        public override void CollectObservations(VectorSensor sensor)
        {
            Vector3 relativePosition = gameObject.transform.position - _target.transform.position;
            float speed = _car_rb.velocity.magnitude;
            Vector3 orientation = gameObject.transform.forward;
        ...
        public override void OnActionReceived(ActionBuffers actionBuffers)
        {
            ...
            if (Vector3.Distance(_target.transform.position, gameObject.transform.position) < 3) {
                print("End Episode");
                EndEpisode();
            }
        }

        private Vector3 ChooseRandomPosition(Vector3 center, float minAngle, float maxAngle, float minRadius, float maxRadius)
        {
        ...
        {
            gameObject.transform.position = ChooseRandomPosition(_spawnArea.transform.position, 0f, 360f, 0f, 3f) + Vector3.up * .5f;
        }
